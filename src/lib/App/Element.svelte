<script lang="ts">
	...
	import { userStore, focusedStore, nodeStore, templateStore } from '$lib/stores/ui/uiStore';
	import { locale, t } from '$lib/stores/translations/i18n';
	...
	const flipDurationMs = 300,
		...
		coreInputClass = 'w-full bg-transparent appearance-none py-1 px-2 outline-none',
	...
	$: editClasses = classNames({
		...
		'focus:outline-dotted':
			($focusedStore === id || (hovered && $focusedStore !== id)) && editorEnabled,
		'outline-dotted': ($focusedStore === id || (hovered && $focusedStore !== id)) && editorEnabled,
    ...
    */
	...
	$: subItems = nodes && id in nodes && 'items' in nodes[id] ? nodes[id].items : [];
</script>
	...
	{#if tagName === 'input' || tagName === 'select' || tagName === 'textarea'}
		<svelte:self tagName="div" {API}>
			...
			<svelte:self
				{API}
				tagName="div"
				className={type === 'range' || type === 'checkbox' || type === 'radio' || type === 'file'
