<script lang="ts">
  ...
  const API = {
    ...
    store: (id) => {
      if ($userStore?.pub) {
        return id &&
          (id.indexOf("~" + $userStore?.pub) === 0 || id.indexOf("_user") === 0)
          ? privateStore
          : publicStore;
      } else {
    ...
    removeUser: (d) =>
      privateStore.deleteUser(d.alias, d.password, (n) =>
        n.err ? API.showError(n.err) : {}
      ),
    showError: (err) => {
      console.log(err);
    },
    ...
    saveTemplate: (e) => {
      ...
      timer = setTimeout(() => {
        ...
        const compressed = encodeURIComponent(
          lz.encodeBase64(lz.compress(string))
        );
        console.log("saving....");
    ...
    toggleElementVisiblity: (d) => {
      if (nodes && d.key in nodes)
        nodes[d.key].layerHidden = !nodes[d.key].layerHidden;
    },
  };
  ...
  const load = () => {
    ...
    document.body.classList.add(
      themeClassPrefix + ($userStore?.dark ? "dark" : "light")
    );
  };
