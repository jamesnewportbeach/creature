		...
		if (templatePath) {
            ...
            */
				...
				API.read(templatePath).once((d) => {
					...
					if (d && lastVersion !== d) {
						lastVersion = d;
						const uncompressed = lz.decompress(lz.decodeBase64(decodeURIComponent(lastVersion)));
...
{#if templatePath in $templateStore}
	<Element bind:nodes on:sendEvent {...$templateStore[templatePath]} {API} {editorEnabled} />
{/if}
