<script>
	import VanillaInfiniteViewer, { OPTIONS, PROPERTIES, EVENTS, CLASS_NAME } from 'infinite-viewer';

	import { onMount, onDestroy, createEventDispatcher, beforeUpdate } from 'svelte';
	...
	onDestroy(() => {
		if (infiniteViewer) infiniteViewer.destroy();
	});
</script>

<div {style} class={`${className || ''} ${CLASS_NAME}`} bind:this={containerElement}>
	<div class="infinite-viewer-wrapper" bind:this={wrapperElement}>
		<div class="infinite-viewer-scroll-area" bind:this={scrollAreaElement} />
		<slot />
	</div>
	<div
	...
	>
		<div class="infinite-viewer-scroll-thumb" />
	</div>
	<div
		class="infinite-viewer-scroll-bar infinite-viewer-vertical-scroll-bar"
		bind:this={verticalScrollElement}
	>
		<div class="infinite-viewer-scroll-thumb" />
	</div>
</div>
