<script lang="ts">
	...
	import { focusedStore } from '$lib/stores/ui/uiStore';
	...
	const consider = (e) => {
			if (node.dragCopy) {
				...
				const newItemIndex = newItems.findIndex(
					(d) => d.isDndShadowItem === true && d.id !== 'id:dnd-shadow-placeholder-0000'
				);
	...
	$: editClasses = classNames({
		...
		'focus:outline-dotted':
			node &&
			($focusedStore === node.id || (node.hovered && $focusedStore !== node.id)) &&
			editorEnabled,
		'outline-dotted':
			node &&
			($focusedStore === node.id || (node.hovered && $focusedStore !== node.id)) &&
		...
		'focus:outline-blue-500': node && $focusedStore === node.id && editorEnabled,
		'dark:focus:outline-lime-500': node && $focusedStore === node.id && editorEnabled,

		'outline-black/25': editorEnabled && node && $focusedStore !== node.id && !node.hovered,
		'outline-black/50': editorEnabled && node && $focusedStore !== node.id && node.hovered,
		'dark:outline-white/25': editorEnabled && node && $focusedStore !== node.id && !node.hovered,
		'dark:outline-white/50': editorEnabled && node && $focusedStore !== node.id && node.hovered,

		// "p-1": editorEnabled && node && node.tagName in ContainerTypes,
		'h-20': editorEnabled && node && 'items' in node && node.items.length === 0,
		'h-full': editorEnabled && node && 'items' in node && node.items.length === 0
	});
  ...
  */
	...
	const newNodes = {
		...
		item: {
			tagName: 'div',
			className: 'flex w-full cursor-pointer select-none px-2 py-1 rounded-lg',
			items: [{ id: 'treeItemLabel' }, { id: 'treeRemoveButton' }, { id: 'treeHideButton' }]
		},
	...
	onMount(() => {
		if (node && node.nodeSource && node.itemRenderer) {
			...
			mergeNodesWithItemRenderer(c, nodes[node.nodeSource], node.itemRenderer, newNodes);
	...
	{:else if 'items' in node}
		...
		>
			{#each node.items as item (item.id)}
				<svelte:self {API} bind:nodes nodeId={item.id} node={nodes[item.id]} on:sendEvent />
				<!--
