<script lang="ts">
	...
	import { AddButtonCellRenderer, DeleteButtonCellRenderer } from '$lib/Graph/components/GridCells';
	...
	const createLabel = (strLabel) => {
		...
		createInput = (type, value) => {
			...
			const inputClasses = 'border p-2 outline-none bg-opacity-30 bg-white rounded-sm w-full';
		...
		formatLabel = (data) => {
			...
			if (end.indexOf(':') > -1) {
				...
				const labelNode = $nodesStore.find((d) => d.id === data.id + '/label/' + lang);
		...
		renderer.prototype.init = function (params) {
			...
			if (params.data) {
				if (params.data.type === 'object') {
					let icon = params.data.icon ? "<i class='far " + params.data.icon + " mr-2'></i>" : '';
					html = "<span class='mr-2' title='" + params.data.id + "''>" + icon + label + '</span>';
				}
				if (params.data.type === 'boolean') {
					let onOff = params.data.value === true ? 'on text-teal-300' : 'off opacity-50';
					html = formWrapper(
						params.data.id,
						label,
				...
				if (params.data.type === 'number') {
					html = formWrapper(params.data.id, label, params.data.value.toLocaleString(lang));
				}
	...
	let dialogSubject, parsedValue, searchDialogOpen, uploadDialogOpen, importContent, importValue;
			...
			timer = setInterval(() => {
				if (api && node.id in objectNodes && 'agGrid' in objectNodes[node.id]) {
					const deleteItems = currentData
						.filter((d) => data.findIndex((item) => item.id.indexOf(d.id) === 0) === -1)
						.map((d) => {
							return {
								id: d.id
							};
					...
					const patch = {
						add: data.filter((d) => currentData.findIndex((item) => item.id === d.id) === -1),
						update: data.filter(
							(d) =>
								currentData.findIndex((item) => item.id === d.id && !isEqual(item, d)) > -1 &&
								deleteItems.findIndex((item) => item.id === d.id) === -1
						),
						remove: deleteItems
					};
					if (patch.add.length + patch.update.length + patch.remove.length > 0) {
						currentData = [...data];
	...
	const options = {
        ...
        */
			...
			valueGetter: (params) => {
				return params.data?.type === 'object' ? params.data.id : params.data?.value;
			},
			valueSetter: (params) => {
		...
		doesExternalFilterPass: (params) => {
			return params.data.isHidden !== true && params.data.id.indexOf('/label') === -1;
		},
      ...
      */
			...
			Object.keys(objectNodes).forEach((graph) => {
				if (objectNodes[graph].agGrid && node.id !== graph) {
					const dropZone = objectNodes[graph].agGrid.gridOptions.api.getRowDropZoneParams();
					api.addRowDropZone(dropZone);
				}
			});
			...
			if (groupStorage) {
				...
				params.api.forEachNode((node) => {
					if (node.group) {
						...
						let ind = groupStorage.findIndex((storageItem) => storageItem === expandedDetails);
						if (ind !== -1) {
        ...
        */
		...
		onRowGroupOpened: (params) => {
			...
			localStorage.setItem(node.id + ':ag-grid', JSON.stringify(expandedNodeDetails));
		},
		...
		onCellValueChanged: (params) => {
			dispatch('nodeEvent', {
				...
				path: params.data.type === 'object' ? params.data.id + '/label/' + lang : params.data.id,
...
<div
	bind:this={ref}
	class="h-full container ag-theme-balham{$privateStore.theme === 'dark' ? '-dark' : ''}"
/>
